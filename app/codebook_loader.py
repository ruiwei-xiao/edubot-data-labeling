"""Load codebook entries from the Google Sheet tab (public CSV export)."""
...
DATA_DIR = ROOT / "data"
CACHE_PATH = DATA_DIR / "cache" / "codebook_sheet.json"

DEFAULT_CODEBOOK_SHEET_ID = "1XwkVPvlQ-kOys8OBn0jTEgYWme8YNqoibVLpkH_tED4"
DEFAULT_CODEBOOK_TAB = "codebook"
SHEET_CODEBOOK_ID = "google-sheet"

# Column indices when using csv.reader (positional).
COL_ASPECT = 0
COL_CODE = 1
COL_DEFINITION = 2
COL_SECONDARY = 3
...
def sheet_id() -> str:
    return os.environ.get("GOOGLE_CODEBOOK_SHEET_ID", DEFAULT_CODEBOOK_SHEET_ID)
    ...
    for idx, raw in enumerate(header):
        ...
        for field, aliases in HEADER_ALIASES.items():
            if norm in aliases and field not in mapping:
                mapping[field] = idx
    # Positional fallback when the aspect column has an empty header.
...
def build_entries_from_csv_text(text: str) -> list[dict[str, Any]]:
    """Parse the codebook sheet into normalized entry dicts (pre-_normalize_entry)."""
...
def build_sheet_cache_payload(entries: list[dict[str, Any]], *, tab: Optional[str] = None) -> dict[str, Any]:
    return {
        "sheet_id": sheet_id(),
        "tab": tab or sheet_tab(),
        "codebook_id": SHEET_CODEBOOK_ID,
        "name": tab or sheet_tab(),
        "entries": entries,
    }
...
def entries_to_sheet_rows(entries: list[dict[str, Any]]) -> list[list[str]]:
    """Serialize entries into sheet rows (aspect shown once per group)."""
    from app.codebook import FIELD_LABELS, _sorted_entries

    rows: list[list[str]] = []
    current_aspect = None
    for entry in _sorted_entries(entries):
        aspect_key = entry.get("aspect") or (entry.get("fields") or [""])[0]
        aspect_label = (
            entry.get("aspect_label")
            or FIELD_LABELS.get(aspect_key)
            or aspect_key.replace("_", " ").title()
        )
